...
file = open("BlueCoin.bin","rb")                #открываем файл прошивки
Sector_Num = 5                                  #Кол-во секторов для стирания в STM32 на плате BlueCoin
PCB_Name = "PCB_NAME = BlueCoin-V.1.0\r\n"      #Ответ от платы 
...
if __name__ == '__main__':
    ...
    for COM_Index in range(len(COM_List)):        
        ...
        if (response == PCB_Name):
            ...
            print(" ")
            COM_BlueCoin = serial.Serial(COM_List[COM_Index], 921600,  timeout = 3)
            break
        elif (COM_Index >= (len(COM_List) - 1)):
            print("No BlueCoin PCB found")
            sys.exit()
    
    time.sleep(0.2)
    
    COM_BlueCoin.write("CMD:RESTART\r\n".encode('ascii'))     # Делаем сброс STM32
    ...
    time.sleep(0.2)
    COM_BlueCoin.write("CMD:ERASE\r\n".encode('ascii'))       # Команда для стирания флэш памяти STM32
    time.sleep(0.2)
    print("Erase in process. Pls wait a wef seconds.")
    for i in range(Sector_Num + 1):
        sector = COM_BlueCoin.readline().decode(encoding = 'UTF-8')
    ...
    COM_BlueCoin.write(fw_size.encode('ascii'))           #отправляем сообщение объёма прошивки в байтах
    
    COM_BlueCoin.readline()
    ...
    for pkg_num in range(num_parts):
        try:
            COM_BlueCoin.write(cmd_list[pkg_num].encode('ascii'))
            response = COM_BlueCoin.readline()
            ...
            sys.exit()
    file.close()
    print(ser.readline().decode(encoding = 'UTF-8')) #получаем последний ответ UPDATE OK! Starting new firmware значит прошивка STM32 прошла успешно
